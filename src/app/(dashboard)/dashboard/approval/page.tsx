...
export default async function ApprovalPage({ searchParams }: ApprovalPageProps) {
  ...
  return (
    <section className="mx-auto flex w-full max-w-6xl flex-col gap-5">
      <div>
        <h2 className="text-lg font-semibold">Waiting Approval</h2>
        <p className="mt-1 text-sm text-muted-foreground">
          Submitted reimbursements from your direct reports.
        </p>
      </div>
        ...
        <CardContent className="space-y-4">
          ...
          {result.data.length === 0 ? (
            <div className="rounded-md border border-dashed px-6 py-10 text-center">
              <p className="font-medium">No waiting approval</p>
              <p className="mt-1 text-sm text-muted-foreground">
                Submitted requests from direct reports will appear here.
              </p>
            </div>
          ) : (
                ...
                {result.data.map((reimbursement) => (
                  ...
                  >
                    ...
                    <Link
                      ...
                      href={`/dashboard/approval/${reimbursement.id}`}
                    >
                      Review
                    </Link>
                  </div>
